...
<div id="" class="sh_viewleft" style="padding:0;width:auto;">
    <div class="sh_Rtit">热门交易排行</div>
    	<c:if test="${not empty exchangeviewGoods}">
    		<c:set var="index" value="0"/>
    		<c:forEach var="item" items="${exchangeviewGoods}">
	...
	<hr>
    <a href="javascript:;" id="autopbn"><i></i>查看下一页</a>
</div>
...
<div class="shopleft" id="left">
    <div class="sh_lone cl">
        ...
        <c:if test="${not empty hotviewGoods}">
    		<c:forEach var="item" items="${hotviewGoods}">
   				<div class="shwLI">
				    <div class="shliimg">
				    <div class="number" title="先到先得">先到先得</div>
				        <a href="../Index/BuyGoods?goodsId=${item.goodsId}" title="<c:out value="${item.goodsName}"/>">
				        <img src="${item.goodsImagaPath}"></a>
				    </div>
				        <p class="shname"><a href="../Index/BuyGoods?goodsId=${item.goodsId}" target="_blank"><c:out value="${item.goodsName}"/></a></p>
				        <p class="sh_money"><span>竞&nbsp;&nbsp;&nbsp;拍：</span><em><c:out value="${item.goodsPrice}"/></em> 金钱</p>
				        <p class="a_ct" id="vd_11021" style="display:none;">
				            <a href="../Index/BuyGoods?goodsId=${item.goodsId}" target="_blank" style="background-image:url(source/plugin/auction/images/auction.png);">查看详情››
				            </a></p>
				        <p id="time_11021" class="sh_button">开始抢购</p>
				</div>
    		</c:forEach>
    	</c:if>
	</div>
...
<div style="padding-bottom:10px;">
    <img src="../integralmall/image/shop_line.jpg"></div>
        <div class="sh_lone">
            <h2 class="shtitle">
            <a href="../IntegralMallMore?type=before" class="sh_more">更多</a>
            <span>往期交易</span></h2>
            <div class="shword cl">
            <c:if test="${not empty recentviewGoods}">
    		<c:set var="index" value="0"/>
    		<c:forEach var="item" items="${recentviewGoods}">
   				<c:set var="index" value="${index+1}"/>
					    <div class="shwLI">
					        <input type="hidden" id="6863" value="-1019">
					            <div class="shliimg">
					                <a href="../Index/BuyGoods?goodsId=${item.goodsId}" title="<c:out value="${item.goodsName}"/>">
					                    <img src="${item.goodsImagaPath}"></a></div>
					                    <p class="shname"><a href="../Index/BuyGoods?goodsId=${item.goodsId}" target="_blank"><c:out value="${item.goodsName}"/></a></p>
					                    <p class="sh_money"><span>兑&nbsp;&nbsp;&nbsp;换：</span><em><c:out value="${item.goodsPrice}"/></em> 金钱</p>
					                    <p class="a_lt" id="vd_6863" style="display:none;"><a href="../Index/BuyGoods?goodsId=${item.goodsId}" target="_blank">查看详情››</a></p>
					                    <p id="time_6863" class="sh_button2">已经结束</p>
					    </div>
    		</c:forEach>
    	</c:if>
    </div>
